...
def pg_to_sqlite(table: Table):
    ...
    new = table.to_metadata(Base.metadata)
...
class ArchiveWriter:
    ...
    def __enter__(self):
        ...
        self._zip_path = ZipPath(
            ...
            info_order=(self._meta_name, self._db_name),
        )
...
def extract_metadata(path: Union[str, Path]) -> Dict[str, Any]:
    ...
    # we fail if not one of the first record in central directory
    # so we don't have to iter all files to fail
    return json.loads(read_file_in_zip(path, ArchiveWriter._meta_name, "utf8", search_limit=1))
...
def extract_db(
    ...
    with out_path.open("wb") as handle:
        extract_file_in_zip(zip_path, ArchiveWriter._db_name, handle, search_limit=3)
    return out_path
